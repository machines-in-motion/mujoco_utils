...
class MujocoWrapper:
    ...
    def end_effector_forces(self):
        ...
        for i in range(len(cnt_geom2)):
            ...
            cnt_status[eff_id] = 1
            cnt_forces[eff_id] = self.physics.data.contact_force(i).reshape(-1)

            # Setting small torques and forces to zero.
            cnt_forces[eff_id][np.where(np.abs(cnt_forces[eff_id]) < 1e-4)[0]] = 0
